...
function Volunteer() {
  ...
  useEffect(() => {
    setInitiativeChoices((prev) =>
      initiatives.map((ini) => ini.name).concat(prev),
    );
  }, [initiatives]);
  ...
  async function submitForm(e) {
    ...
    const selectedInitiatives =
      document.querySelector("#initiative").dataset.value;

    const name = formData.get("name");
    const email = formData.get("email");
    const phone = formData.get("phone");
    const initiatives = formData
      .getAll("initiative")
      .map((ini, key) => `${key + 1}. ${ini}`)
      .join("<br/>");
...
`;
    ...
    try {
      const docRef = await addDoc(collection(db, "mail"), {
        to: ["prayagjain2@gmail.com", "Khajanchivarsha@gmail.com"],
        message: {
    ...
    <Wrapper id="volunteer" className="text-center py-10 scroll-mt-[10vh]">
      <form id="volunteerForm" onSubmit={(e) => submitForm(e)}>
        ...
        <Textarea
          ...
          label="Have you ever volunteered before? If yes, describe your experience!"
          required
        />
